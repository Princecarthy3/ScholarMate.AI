...
const AuthManager = {
  ...
  async register(email, password, name) {
    ...
    localStorage.setItem('scholarmate_current_user', email);
    localUser.initials = getInitials(localUser.name, localUser.email);
    return { user: localUser, autoLogin: true, supabaseNotice: supabaseErr };
  },
  ...
  async login(email, password) {
    ...
    if (!user) {
      if (supabaseErr && !supabaseErr.includes('Failed to fetch')) {
        // If Supabase failed with explicit credentials error and no local record exists
        throw new Error(supabaseErr + '. If you have not created an account yet, click "Create Account" or use "Instant Guest Access".');
      }
...
`;
    ...
    try {
      const rawText = await callOpenRouterApi(prompt, { requireJson: true });
      ...
      showToast(err.message || 'Could not connect to the AI service. Check your OpenRouter backend configuration.');
...
function setupAuthForms() {
  ...
  if ($('#guestLoginBtn')) {
    $('#guestLoginBtn').onclick = async () => {
      try {
        const guestEmail = 'guest.learner@scholarmate.ai';
        const users = JSON.parse(localStorage.getItem('scholarmate_users') || '{}');
        let guestUser = users[guestEmail] || {
          id: 'guest-local-user',
          email: guestEmail,
          name: 'Scholar Guest',
          streak: 1,
        ...
        users[guestEmail] = guestUser;
        localStorage.setItem('scholarmate_users', JSON.stringify(users));
        localStorage.setItem('scholarmate_current_user', guestEmail);
        guestUser.initials = getInitials(guestUser.name, guestUser.email);
        showToast('✓ Logged in as Scholar Guest!');
        enterApp(guestUser);
      } catch (err) {
        showToast('Guest login error: ' + err.message);
      }
    };
  }
...
async function enterApp(user) {
  ...
  if (supabase && user.id && isUuid(user.id)) {
    ...
    // Local / Guest user state initialization
    updateUIForUser(user);
    hydrateSavedChatsAndNotes(user);
  }
}
...
function setupQuizLab() {
  ...
  async function handleTopicQuizSubmit() {
    ...
    if ($('#quizContainer')) $('#quizContainer').innerHTML = '<div class="glass-card" style="padding:40px;text-align:center;"><p>Synthesizing questions via AI Engine…</p></div>';
...
`;
  ...
  try {
    const rawGuideText = await callOpenRouterApi(prompt);
...
function setupFlashcardsEngine() {
  ...
  if ($('#generateFlashcardsBtn')) {
    $('#generateFlashcardsBtn').onclick = async () => {
      ...
      try {
        const rawText = await callOpenRouterApi(prompt);
